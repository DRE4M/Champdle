...
faq-similarity-description = 유사도는 총 100점 만점(카테고리 7종 80점 + 스탯 18종 20점)을 기준으로 산출됩니다:
  1. 카테고리 항목 (총 80점 / 7종 각 약 11.43점 배점)
  - 소속 지역, 공격 방식, 자원 유형, 성별: 완전히 같으면 만점(100%)을 부여합니다.
  - 종족: 완전히 같으면 100%, 복수 종족 중 하나라도 공통으로 겹치면(예: Human/Cyborg ↔ Human) 절반 점수(50%)를 부여합니다.
  - 역할군 1·2 (Tag): 주/부역할군 순서와 종류 모두 일치 100%, 주역할군만 일치 80%, 주/부역할군 교차 완전 일치 70%, 부역할군 부분 일치 50%를 부여합니다.
  - 출시 순서: 1번(애니)부터 173번까지의 순서 격차(Min-Max)에 따라 비율 점수가 적용됩니다.
  2. 스탯 항목 (총 20점 / 18종 각 약 1.11점 배점)
  - 이동속도, 사거리, 체력, 마나, 공격력, 방어력, 마저, 재생력 등 18개 능력치의 수치 차이를 Min-Max 정규화 공식으로 산출합니다.
